...
def to_iconify_id(icon: str) -> str:
    """Full Iconify id for the <iconify-icon> web component. A bare name is a
    Bootstrap icon (`bi` set); `set:name` (`logos:snowflake-icon`) is used
    as-is. Monochrome icons inherit the sidebar text colour automatically (the
    component renders them with currentColor); the multicolour `logos` set
